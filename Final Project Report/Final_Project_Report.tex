...
\title{EECS/CSE31L Final Project Report\\
Group name: MITTT\\
{\footnotesize Github repo: \href{https://github.com/minabedwany/Final_Project}{Final\_Project}}}

\author{Mina Bedwany {$\ $\footnotesize69543570}, Tianyi Yang {$\ $\footnotesize69386993}, Takumi Okamoto {$\ $\footnotesize86483191}}
...
\begin{document}
	...
	This Final Project is a RISC-V processor which combines all the lab modules we have done throughout these several weeks. The structure of this RISC-V processor is basically three parts: \textbf{Datapath, Controller, and ALU control modules}. The arrangement of the processor is shown below.
	
	\begin{figure}[h]
		\centering
		\includegraphics[width=14cm, height=17.6cm]{"Block_diagram".pdf}% no need to explicitly specify the file extension
	...
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			\begin{description}
				...
				\item[\texttt{MemWrite}] Write data input is written into \texttt{Mem[ addr ]}
								 
				\item[\texttt{ALUOp}] Determine functionality of the ALU unit
			\end{description}
			
			\item The ALU controller takes the operation code passed by the controller. Depends on the \texttt{ALUop}, it takes two segments from the instruction memory called \texttt{"Func3"} and \texttt{"Func7"}. Finally, it will generate a \textbf{ALU Control Code} and pass it to Datapath.
			
			\item The Datapath takes in total 8 inputs. Two of the input are \texttt{clk} from the \texttt{processor\_clk} and \texttt{rst} from the \texttt{btnR}. For the rest of six inputs, five of them are provided by controller and the last on is from the ALU controller. Finally, the Datapath will update the Instruction Memory and pass another 64-bits output \texttt{ALU\_result}.
		\end{enumerate}
	
		\item The last module \texttt{seg\_display} uses the clock signal from \texttt{seg\_disp\_clk}, the reset signal from \texttt{btnR}, and the first 16-bits from the \texttt{ALU\_result} to process and generate our two last output \texttt{seg} and \texttt{an}.
	\end{enumerate}
	...
	\subsection{Our Instruction Set for Testing}
	\begin{minted}
	[
	frame=lines,
	framesep=2mm,
...
assign rd =  Inst_mem [ra[INS_ADDRESS-1:2]];  
	...
	\begin{figure}[h]
		\centering
		\includegraphics[width=14cm, height=7.5cm]{Simulation_set1_hex.jpg}
		\caption{Simulated Waveform for Instruction Set 1}
	\end{figure}

	\begin{figure}[h]
		\centering
		\includegraphics[width=14cm, height=7.5cm]{Simulation_set2_decimal.jpg}
		\caption{Simulated Waveform for Instruction Set 2}
	\end{figure}

	\begin{figure}[h]
		\centering
		\includegraphics[width=14cm, height=7.5cm]{Simulation_set3_decimal.jpg}
		\caption{Simulated Waveform for Instruction Set 3}
	\end{figure}
\end{document}
